...
def my_dag():
    ...
    for partners, details in partners.items():
        ...
        @task.python(task_id=f"extract_{partners}",retries=3,on_success_callback=_extract_callback_sucess,on_failure_callback=_extract_callback_failure, on_retry_callback=_extract_callback_retry,depends_on_past=True, priority_weight=details['priority'], do_xcom_push=False, pool=details['pool'],multiple_outputs=True)
